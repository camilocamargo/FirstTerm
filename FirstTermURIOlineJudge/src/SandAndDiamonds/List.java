...
//    }
    ...
    public void searchForNumber(String number) throws IOException {
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
        Node temp = head;
        while (!String.valueOf(temp.character).equals(number) && temp.next != null) {
            temp = temp.next;
        }
        if (String.valueOf(temp.character).equals(number)) {
            bw.write("Number: " + temp.character+ "\n");
            bw.flush();
        } else {
    ...
    public Node binarySearch(int number){ //falta arreglar cuando el dato a buscar no existe
        ...
        while(middle != lower){
            ...
            if(temp.character == number){
                return temp;
            }
            else if(number > temp.character){
                lower = middle;
            }else{
                higher = middle;
                temp = head;
